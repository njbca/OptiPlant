...
# =============================================
# Hourly Series Dashboard
# ---------------------------------------------
# This dashboard allows users to select a hourly scenario CSV file,
...
default_folder = Path.cwd() / "results" / "Example" / "Results_example" / "Hourly results" / "Flows"

# Folder path input field (in the main body)
flows_results_folder = st.text_input(
    "Enter 'Flows' folder path:",
    value=str(default_folder)  # pre-fill with default
)

if flows_results_folder:  # Only run if user entered something
    folder_path = Path(flows_results_folder)  # Convert input to a Path object
    if folder_path.exists() and folder_path.is_dir():  # Check if it's a valid directory
        # Get all CSV files in the folder
        csv_files = [p.name for p in sorted(folder_path.glob('*.csv'))]
        
        if csv_files:
            st.success(f"Found {len(csv_files)} scenario CSV files in the folder")
            
            # Scenario selection dropdown in the sidebar
            selected_file = st.sidebar.selectbox("Select scenario:", csv_files)
            
            # Read the selected CSV file
            csv_path = folder_path / selected_file
            df = pd.read_csv(csv_path)
        
        else:
            st.warning("No CSV files found in the folder.")
    else:
        st.error(f"The folder '{flows_results_folder}' does not exist or is not a valid directory.")
...
# Main plot area
st.title(f"Hourly timeseries dashboard - {selected_file}")
if selected_series:
    # Create the plot
